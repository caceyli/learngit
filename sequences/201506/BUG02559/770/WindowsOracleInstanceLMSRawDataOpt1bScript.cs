...
{
    public class WindowsOracleInstanceLMSRawDataOpt1bScript : ICollectionScriptRuntime
    {
                ...
                ITftpDispatcher tftpDispatcher)
        {
            ...
            Lib.Logger.TraceEvent(TraceEventType.Start,
                                  0,
                                  "Task Id {0}: Collection script WindowsOracleInstanceLMSRawDataOpt1bScript.",
                                  m_taskId);
            ...
            try
            {
                ...
                if (connection == null)
                {
                    ...
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Connection object passed to WindowsOracleInstanceLMSRawDataOpt1bScript is null.",
                                          m_taskId);
                }
            ...
            catch (Exception ex)
            {
                if (ResultCodes.RC_SUCCESS == resultCode)
                {
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Unhandled exception in WindowsOracleInstanceLMSRawDataOpt1bScript.  Elapsed time {1}.\n{2}\nResult code changed to RC_PROCESSING_EXCEPTION.",
                                          m_taskId,
            ...
            Lib.Logger.TraceEvent(TraceEventType.Stop,
                                  0,
                                  "Task Id {0}: Collection script WindowsOracleInstanceLMSRawDataOpt1bScript.  Elapsed time {1}.  Result code {2}.",
                                  m_taskId,
                                  m_executionTimer.Elapsed.ToString(),
                                  resultCode.ToString());
        ...
        private string buildBatchFile(string strTempDir, string strHostName, string strDBName, string strOracleHome, string strSchemaName, string strSchemaPassword)
        {
            ...
            strBatchFile.Append("ECHO define TNS_NAME='';")
                        .Append(@" >> ").Append(strTempDir).AppendLine(@"\%1\QUERY.SQL");
            strBatchFile.Append("ECHO DROP TABLE LMS_OPTIONS1B;")
            .Append(@" >> ").Append(strTempDir).AppendLine(@"\%1\QUERY.SQL");
            strBatchFile.Append("ECHO CREATE TABLE LMS_OPTIONS1B(MACHINE_ID VARCHAR2(255), DB_NAME VARCHAR2(255), ")
        ...
        private static void RawDataValueHandler
            (WindowsOracleInstanceLMSRawDataOpt1bScript scriptInstance, String attributeName, String queryOutput)
        {
        ...
        /// </summary>
        private delegate void QueryResultHandler(WindowsOracleInstanceLMSRawDataOpt1bScript scriptInstance, string attributeName, string outputData);
        ...
        /// </summary>
        static WindowsOracleInstanceLMSRawDataOpt1bScript()
        {
        ...
        private static readonly KeyValuePair<string, QueryTableEntry>[] s_queryTable = {
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS5",            
...
.Append(" INSTANCE_NAME=[&INSTANCE_NAME.] DBA_USERS_SOURCE=[&DBA_USERS_SOURCE.] ")
.AppendLine(" ")
.AppendLine(" prompt ")
.AppendLine("define OPTION_NAME=PARTITIONING ")
.AppendLine("define OPTION_QUERY=PARTITIONED_SEGMENTS ")
.AppendLine(" DECLARE ")
...
.AppendLine("      min(CREATED), ")
.AppendLine("      min(LAST_DDL_TIME) ")
.AppendLine("    from DBA_OBJECTS&DBL ")
.AppendLine("      where OBJECT_TYPE LIKE ''%PARTITION%'' ")
.AppendLine("      group by OWNER, OBJECT_TYPE, OBJECT_NAME ")
.AppendLine("  '); ")
.AppendLine("  mycount := SQL%ROWCOUNT; ")
.AppendLine("  -- Recording 'no rows selected' case ")
.AppendLine("  IF mycount=0 THEN ")
.AppendLine("    INSERT INTO LMS_OPTIONS1B ")
.AppendLine("    ( MACHINE_ID, DB_NAME, TIMESTAMP, HOST_NAME, INSTANCE_NAME, OPTION_NAME, OPTION_QUERY, ")
...
.AppendLine("    insert into LMS_OPTIONS1B (select * from TMP_LMS_OPTIONS); ")
.AppendLine("  END IF; ")
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"lmsOptions1b",           
         new QueryTableEntry(new StringBuilder("SELECT '<BDNA=>MACHINE_ID='||MACHINE_ID||'<BDNA,1>DB_NAME='||DB_NAME||'<BDNA,1>TIMESTAMP='||TIMESTAMP" +
                  ...
                  @"||'<BDNA,1>COL100='||COL100||'<=BDNA>' FROM LMS_OPTIONS1B "),
         all_ver_pattern,
         new QueryResultHandler(RawDataQueryValueHandler))),

         new KeyValuePair<string, QueryTableEntry>(@"dropLMSOptions",            
         new QueryTableEntry(new StringBuilder("DROP TABLE LMS_OPTIONS1B;"),                              
         all_ver_pattern,
         new QueryResultHandler(RawDataValueHandler))),
