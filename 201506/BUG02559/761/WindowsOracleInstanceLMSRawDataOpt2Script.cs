...
{
    public class WindowsOracleInstanceLMSRawDataOpt2Script : ICollectionScriptRuntime
    {
                ...
                ITftpDispatcher tftpDispatcher)
        {
            ...
            Lib.Logger.TraceEvent(TraceEventType.Start,
                                  0,
                                  "Task Id {0}: Collection script WindowsOracleInstanceLMSRawDataOpt2Script.",
                                  m_taskId);
            ...
            try
            {
                ...
                if (connection == null)
                {
                    ...
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Connection object passed to WindowsOracleInstanceLMSRawDataOpt2Script is null.",
                                          m_taskId);
                }
            ...
            catch (Exception ex)
            {
                if (ResultCodes.RC_SUCCESS == resultCode)
                {
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Unhandled exception in WindowsOracleInstanceLMSRawDataOpt2Script.  Elapsed time {1}.\n{2}\nResult code changed to RC_PROCESSING_EXCEPTION.",
                                          m_taskId,
            ...
            Lib.Logger.TraceEvent(TraceEventType.Stop,
                                  0,
                                  "Task Id {0}: Collection script WindowsOracleInstanceLMSRawDataOpt2Script.  Elapsed time {1}.  Result code {2}.",
                                  m_taskId,
                                  m_executionTimer.Elapsed.ToString(),
                                  resultCode.ToString());
        ...
        private string buildBatchFile(string strTempDir, string strHostName, string strDBName, string strOracleHome, string strSchemaName, string strSchemaPassword)
        {
            ...
            strBatchFile.Append("ECHO define TNS_NAME='';")
                        .Append(@" >> ").Append(strTempDir).AppendLine(@"\%1\QUERY.SQL");
            strBatchFile.Append("ECHO DROP TABLE LMS_OPTIONS2;")
            .Append(@" >> ").Append(strTempDir).AppendLine(@"\%1\QUERY.SQL");

            strBatchFile.Append("ECHO CREATE TABLE LMS_OPTIONS2(MACHINE_ID VARCHAR2(255), DB_NAME VARCHAR2(255), ")
        ...
        private static void RawDataValueHandler
            (WindowsOracleInstanceLMSRawDataOpt2Script scriptInstance, String attributeName, String queryOutput)
        {
        ...
        /// </summary>
        private delegate void QueryResultHandler(WindowsOracleInstanceLMSRawDataOpt2Script scriptInstance, string attributeName, string outputData);
        ...
        /// </summary>
        static WindowsOracleInstanceLMSRawDataOpt2Script()
        {
        ...
        private static readonly KeyValuePair<string, QueryTableEntry>[] s_queryTable = {
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS11",            
...
.AppendLine(" prompt ")
.AppendLine(" ------------------------ ")
.AppendLine(" -- OEM 10G AND HIGHER -- " )
.AppendLine(" ------------------------ " )
.AppendLine(" define OPTION_NAME=OEM " )
.AppendLine(" define OPTION_QUERY=MGMT_ADMIN_LICENSES " )
.AppendLine(" define OEMOWNER=SYSMAN " )
.AppendLine(" col OEMOWNER new_val OEMOWNER format a30 wrap noprint " )
.AppendLine(" select owner as OEMOWNER from dba_tables&DBL where table_name = " )
.AppendLine(" 'MGMT_ADMIN_LICENSES'; " )
.AppendLine(" DECLARE " )
...
.AppendLine(" BEGIN " )
.AppendLine(" begin " )
.AppendLine(" execute immediate ('insert into lms_options2 " )
.AppendLine(" ( " )
...
.AppendLine(" ''&OPTION_NAME.'', " )
.AppendLine(" ''&OPTION_QUERY.'', " )
.AppendLine(" b.pack_name, " )
.AppendLine(" a.pack_label, " )
.AppendLine(" a.target_type, " )
.AppendLine(" a.pack_display_label, " )
.AppendLine(" c.PACK_ACCESS_AGREED " )
.AppendLine(" FROM " )
.AppendLine(" &&OEMOWNER..MGMT_LICENSE_DEFINITIONS&DBL a, " )
.AppendLine(" &&OEMOWNER..MGMT_ADMIN_LICENSES&DBL b, " )
.AppendLine(" (SELECT DECODE(COUNT(*), 0, ''NO'',''YES'') AS PACK_ACCESS_AGREED " )
.AppendLine(" FROM &&OEMOWNER..MGMT_LICENSES&DBL where upper(I_AGREE)=''YES'') c " )
.AppendLine(" WHERE a.pack_label = b.pack_name (+)'); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
.AppendLine(" myerr := SQLERRM; " )
.AppendLine(" INSERT INTO LMS_OPTIONS2 " )
.AppendLine(" (MACHINE_ID, " )
.AppendLine(" DB_NAME, " )
.AppendLine(" TIMESTAMP, " )
.AppendLine(" HOST_NAME, " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS12",            
         new QueryTableEntry(new StringBuilder(" ----------------------------------------- " )
.AppendLine()
.AppendLine(" -- OEM PACK ACCESS AGREEMENTS (10g or higher) -- " )
.AppendLine(" ------------------------------------------------ " )
.AppendLine(" define OPTION_NAME=OEM " )
.AppendLine(" define OPTION_QUERY=MGMT_LICENSES " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" I_AGREE " )
.AppendLine(" FROM " )
.AppendLine(" &&OEMOWNER..MGMT_LICENSES&DBL'); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
.AppendLine(" myerr := SQLERRM; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS13",            
         new QueryTableEntry(new StringBuilder(" ------------------------------------------- " )
.AppendLine()
.AppendLine(" -- OEM MANAGED DATABASES (10g or higher) -- " )
.AppendLine(" ------------------------------------------- " )
.AppendLine(" define OPTION_NAME=OEM " )
.AppendLine(" define OPTION_QUERY=MGMT_TARGETS " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" HOST_NAME, " )
.AppendLine(" LOAD_TIMESTAMP " )
.AppendLine(" FROM " )
.AppendLine(" &&OEMOWNER..MGMT_TARGETS&DBL " )
.AppendLine(" WHERE TARGET_TYPE = ''oracle_database'''); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
.AppendLine(" myerr := SQLERRM; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS141",            
         new QueryTableEntry(new StringBuilder(" --------------------------------------------------- " )
.AppendLine()
.AppendLine(" -- OEM MGMT_LICENSE_CONFIRMATION (10g or higher) -- " )
.AppendLine(" --------------------------------------------------- " )
.AppendLine(" define OPTION_NAME=OEM " )
.AppendLine(" define OPTION_QUERY=MGMT_LICENSE_CONFIRMATION " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" ''&OPTION_NAME.'', " )
.AppendLine(" ''&OPTION_QUERY.'', " )
.AppendLine(" a.CONFIRMATION, " )
.AppendLine(" a.CONFIRMED_BY, " )
.AppendLine(" a.CONFIRMED_TIME, " )
.AppendLine(" b.TARGET_NAME, " )
.AppendLine(" b.TARGET_TYPE, " )
.AppendLine(" b.TYPE_DISPLAY_NAME " )
.AppendLine(" FROM " )
.AppendLine(" &&OEMOWNER..MGMT_LICENSE_CONFIRMATION&DBL a, " )
.AppendLine(" &&OEMOWNER..MGMT_TARGETS&DBL b " )
.AppendLine(" WHERE a.TARGET_GUID = b.TARGET_GUID (+)'); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
.AppendLine(" myerr := SQLERRM; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS142",            
         new QueryTableEntry(new StringBuilder(" --------------------------------------------- " )
.AppendLine()
.AppendLine(" -- OEM SQL PROFILES CHECKS (10g or higher) --" )
.AppendLine(" ---------------------------------------------" )
.AppendLine(" define OPTION_NAME=OEM" )
.AppendLine(" define OPTION_QUERY=SQL_PROFILES" )
.AppendLine(" DECLARE" )
.AppendLine("         mycode number;" )
.AppendLine("         myerr varchar2 (3000);" )
.AppendLine(" BEGIN" )
.AppendLine("   -- Using dynamic SQL to count rows" )
.AppendLine("   EXECUTE IMMEDIATE ('select count(*)" )
.AppendLine("    from dba_sql_profiles&DBL" )
.AppendLine("    where lower(status) like ''enabled''" )
.AppendLine("   ')" )
.AppendLine("   INTO mycode" )
.AppendLine("   ;" )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS143",            
         new QueryTableEntry(new StringBuilder(" --------------------------- " )
.AppendLine()
.AppendLine(" -- OEM 10g Grid Control  -- " )
.AppendLine(" --------------------------- " )
.AppendLine(" define OPTION_NAME=OEM " )
.AppendLine(" define OPTION_QUERY=GRID_CONTROL+11g " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" ''&OPTION_NAME.'', " )
.AppendLine(" ''&OPTION_QUERY.'', " )
.AppendLine(" tt.type_display_name, " )
.AppendLine(" tg.host_name, " )
.AppendLine(" tg.target_name , " )
.AppendLine(" ld.pack_display_label, " )
.AppendLine(" decode(lt.pack_name , null, ''NO'', ''YES''), " )
.AppendLine(" decode(lc.target_guid, null, ''NO'', ''YES'') , " )
.AppendLine(" lc.confirmed_time, " )
.AppendLine(" lc.confirmed_by, " )
.AppendLine(" tg.target_type, " )
.AppendLine(" ld.pack_label " )
.AppendLine(" from &&OEMOWNER..MGMT_TARGETS&DBL tg " )
.AppendLine(" left outer join &&OEMOWNER..MGMT_TARGET_TYPES&DBL tt on tg.target_type = " )
.AppendLine(" tt.target_type " )
.AppendLine(" inner join &&OEMOWNER..MGMT_LICENSE_DEFINITIONS&DBL ld on tg.target_type = " )
.AppendLine(" ld.target_type " )
.AppendLine(" left outer join &&OEMOWNER..MGMT_LICENSED_TARGETS&DBL lt on tg.target_guid = " )
.AppendLine(" lt.target_guid and ld.pack_label = lt.pack_name " )
.AppendLine(" left outer join &&OEMOWNER..MGMT_LICENSE_CONFIRMATION&DBL lc on tg.target_guid " )
.AppendLine(" = lc.target_guid " )
.AppendLine(" order by tg.host_name, tt.type_display_name, tg.target_name, " )
.AppendLine(" ld.pack_display_label '); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS15",            
         new QueryTableEntry(new StringBuilder(" ------------- " )
.AppendLine()
.AppendLine(" -- SPATIAL -- " )
.AppendLine(" ------------- " )
.AppendLine("define  OPTION_NAME=SPATIAL ")
.AppendLine("define OPTION_QUERY=ALL_SDO_GEOM_METADATA ")
.AppendLine("DECLARE ")
.AppendLine("        mycode number; ")
.AppendLine("        myerr varchar2 (3000); ")
...
.AppendLine("    select ")
.AppendLine("      ''&MACHINE_ID.'', ''&DB_NAME.'', ''&SYS_TIME.'', ''&HOST_NAME.'', ''&INSTANCE_NAME.'', ''&OPTION_NAME.'', ''&OPTION_QUERY.'', ")
.AppendLine("      SDO_OWNER, ")
.AppendLine("      SDO_TABLE_NAME, ")
.AppendLine("      substr(SDO_COLUMN_NAME, 1, 250) ")
.AppendLine("    from MDSYS.SDO_GEOM_METADATA_TABLE&DBL ")
.AppendLine("  '); ")
...
.AppendLine("    insert into LMS_OPTIONS2 (select * from TMP_LMS_OPTIONS); ")
.AppendLine("  END IF; ")
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS16",            
         new QueryTableEntry(new StringBuilder(" ------------------------ " )
.AppendLine()
.AppendLine(" -- DATA MINING -- " )
.AppendLine(" ----------------- " )
.AppendLine(" -- 9i -- " )
.AppendLine(" -------- " )
.AppendLine(" define OPTION_NAME=DATA_MINING " )
.AppendLine(" define OPTION_QUERY=09i.ODM_MINING_MODEL " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" count(*) " )
.AppendLine(" FROM " )
.AppendLine(" ODM.ODM_MINING_MODEL&DBL'); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
.AppendLine(" myerr := SQLERRM; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS17",            
         new QueryTableEntry(new StringBuilder(" ----------- " )
.AppendLine()
.AppendLine(" -- 10gv1 -- " )
.AppendLine(" ----------- " )
.AppendLine(" define OPTION_NAME=DATA_MINING " )
.AppendLine(" define OPTION_QUERY=10gv1.DM$OBJECT " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" count(*) " )
.AppendLine(" FROM " )
.AppendLine(" DMSYS.DM$OBJECT&DBL'); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
.AppendLine(" myerr := SQLERRM; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS18",            
         new QueryTableEntry(new StringBuilder(" ----------- " )
.AppendLine()
.AppendLine(" -- 10gv2 -- " )
.AppendLine(" ----------- " )
.AppendLine(" define OPTION_NAME=DATA_MINING " )
.AppendLine(" define OPTION_QUERY=10gv2.DM$P_MODEL " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" count(*) " )
.AppendLine(" FROM " )
.AppendLine(" DMSYS.DM$P_MODEL&DBL'); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
.AppendLine(" myerr := SQLERRM; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS181",            
         new QueryTableEntry(new StringBuilder(" --------- " )
.AppendLine()
.AppendLine(" -- 11g -- " )
.AppendLine(" --------- " )
.AppendLine(" define OPTION_NAME=DATA_MINING " )
.AppendLine(" define OPTION_QUERY=11g.DM$P_MODEL " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS182",
         new QueryTableEntry(new StringBuilder(" ------------------------------------------- " )
.AppendLine()
.AppendLine(" define OPTION_QUERY=11g+.DBA_MINING_MODELS ")
.AppendLine(" DECLARE ")
.AppendLine("         mycode number; ")
.AppendLine("         myerr varchar2 (3000); ")
.AppendLine("         mycount number; ")
...
.AppendLine("       MINING_FUNCTION , ")
.AppendLine("       ALGORITHM       , ")
.AppendLine("       CREATION_DATE   , ")
.AppendLine("       BUILD_DURATION  , ")
.AppendLine("       MODEL_SIZE ")
.AppendLine("     from SYS.DBA_MINING_MODELS&DBL ")
.AppendLine("     order by OWNER, MODEL_NAME ")
.AppendLine("   '); ")
.AppendLine("   mycount := SQL%ROWCOUNT; ")
.AppendLine("   -- Recording 'no rows selected' case ")
.AppendLine("   IF mycount=0 THEN ")
...
.AppendLine("END; "),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"LMS_OPTIONS20",            
         new QueryTableEntry(new StringBuilder(" -------------------- " )
.AppendLine()
.AppendLine(" -- DATABASE VAULT -- " )
.AppendLine(" -------------------- " )
.AppendLine(" define OPTION_NAME=DATABASE_VAULT " )
.AppendLine(" define OPTION_QUERY=DVSYS_SCHEMA " )
.AppendLine(" DECLARE " )
.AppendLine(" mycode number; " )
.AppendLine(" myerr varchar2 (3000); " )
.AppendLine(" does_not_exist exception; " )
...
.AppendLine(" WHERE UPPER(username)=''DVSYS'''); " )
.AppendLine(" EXCEPTION " )
.AppendLine(" when OTHERS then " )
.AppendLine(" mycode := SQLCODE; " )
.AppendLine(" myerr := SQLERRM; " )
...
.AppendLine(" END; " ),
         ...
         new KeyValuePair<string, QueryTableEntry>(@"lmsOptions2",           
         new QueryTableEntry(new StringBuilder("SELECT '<BDNA=>MACHINE_ID='||MACHINE_ID||'<BDNA,1>DB_NAME='||DB_NAME||'<BDNA,1>TIMESTAMP='||TIMESTAMP" +
                  ...
                  @"||'<BDNA,1>COL100='||COL100||'<=BDNA>' FROM LMS_OPTIONS2 "),
         all_ver_pattern,
         new QueryResultHandler(RawDataQueryValueHandler))),

         new KeyValuePair<string, QueryTableEntry>(@"dropLMSOptions",            
         new QueryTableEntry(new StringBuilder("DROP TABLE LMS_OPTIONS2;"),                              
         all_ver_pattern,
         new QueryResultHandler(RawDataValueHandler))),
