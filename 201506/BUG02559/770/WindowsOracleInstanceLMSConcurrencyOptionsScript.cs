...
{
    public class WindowsOracleInstanceLMSConcurrencyOptionsScript : ICollectionScriptRuntime
    {
                ...
                ITftpDispatcher tftpDispatcher)
        {
            ...
            Lib.Logger.TraceEvent(TraceEventType.Start,
                                  0,
                                  "Task Id {0}: Collection script WindowsOracleInstanceLMSConcurrencyOptionsScript.",
                                  m_taskId);
            ...
            try
            {
                ...
                if (connection == null)
                {
                    ...
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Connection object passed to WindowsOracleInstanceLMSConcurrencyOptionsScript is null.",
                                          m_taskId);
                }
                ...
                if (resultCode == ResultCodes.RC_SUCCESS)
                {
                    ...
                    if (resultCode == ResultCodes.RC_SUCCESS && stdoutData.Length > 0)
                    {
                        ...
                        this.processVSessionCollectedData();
                        this.processDBAUsersCollectedData();
                        foreach (KeyValuePair<string, string> kvp in m_collectedData)
                        {
                            this.BuildDataRow(kvp.Key, kvp.Value);
                        }
                        //Console.WriteLine(CollectedData[@"lmsVLicense"]);
                        //Console.WriteLine(CollectedData[@"lmsDBAUsers"]);
                        //Console.WriteLine(CollectedData[@"lmsDBName"]);
                        //Console.WriteLine(CollectedData[@"lmsUserCount"]);                        
                        //Console.WriteLine(CollectedData[@"lmsCollectTime"]);   
                    }
                }
            }
            catch (Exception ex)
            {
                if (ResultCodes.RC_SUCCESS == resultCode)
                {
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Unhandled exception in WindowsOracleInstanceLMSConcurrencyOptionsScript.  Elapsed time {1}.\n{2}\nResult code changed to RC_PROCESSING_EXCEPTION.",
                                          m_taskId,
            ...
            Lib.Logger.TraceEvent(TraceEventType.Stop,
                                  0,
                                  "Task Id {0}: Collection script WindowsOracleInstanceLMSConcurrencyOptionsScript.  Elapsed time {1}.  Result code {2}.",
                                  m_taskId,
                                  m_executionTimer.Elapsed.ToString(),
                                  resultCode.ToString());
        ...
        /// <summary>
        /// Process VLicense collected Result.
        /// </summary>
        private void processVLicenseCollectedData()
        {
            StringBuilder result = new StringBuilder();
            if (CollectedData.ContainsKey(@"lmsMachineID"))
            {
                if (result.Length > 0)
                {
                    result.Append("<BDNA,1>");
                }
                result.Append(@"MACHINE_ID=").Append(CollectedData[@"lmsMachineID"]);
            }

            if (CollectedData.ContainsKey(@"lmsDBName"))
            {
                if (result.Length > 0)
                {
                    result.Append("<BDNA,1>");
                }
                result.Append(@"DB_NAME=").Append(CollectedData[@"lmsDBName"]);
            }

            if (CollectedData.ContainsKey(@"VLicense")) 
            {
                if (result.Length > 0)
                {
                    result.Append("<BDNA,1>");
                }
                result.Append(CollectedData[@"VLicense"]);                
            }
            else if (CollectedData.ContainsKey(@"VLicense_9")) {
                if (result.Length > 0)
                {
                    result.Append("<BDNA,1>");
                }
                result.Append(CollectedData[@"VLicense_9"]);
            }
            CollectedData[@"lmsVLicense"] = result.ToString();
        }

        /// <summary>
        /// Process VSession collected Result.
        /// </summary>
        private void processVSessionCollectedData()
        {
            string host = null;
            string db = null;
            StringBuilder result = new StringBuilder();
            if (CollectedData.ContainsKey(@"lmsMachineID"))
            {
                host = CollectedData[@"lmsMachineID"];
            }

            if (CollectedData.ContainsKey(@"lmsDBName"))
            {
                db = CollectedData[@"lmsDBName"];
            }

            if (!string.IsNullOrEmpty(CollectedData[@"VSession"]))
            {
                foreach (string line in CollectedData[@"VSession"].Split(new string[] { "<BDNA,>" }, StringSplitOptions.RemoveEmptyEntries))
                {                    
                    if (result.Length > 0)
                    {
                        result.Append("<BDNA,>");
                    }
                    result.Append(@"MACHINE_ID=").Append(host)
                          .Append(@"<BDNA,1>DB_NAME=").Append(db)
                          .Append(@"<BDNA,1>").Append(line);
                }                
            }
            CollectedData[@"lmsVSession"] = result.ToString();
        }

        /// <summary>
        /// Process DBA Users collected Result.
        /// </summary>
        private void processDBAUsersCollectedData()
        {
            string host = null;
            string db = null;
            StringBuilder result = new StringBuilder();
            ...
            if (!string.IsNullOrEmpty(CollectedData[@"DBAUsers"]))
            {               
                foreach (string line in CollectedData[@"DBAUsers"].Split(new string[] { "<BDNA,>" }, StringSplitOptions.RemoveEmptyEntries))
                {
                    if (result.Length > 0)
                    {
                        result.Append("<BDNA,>");
            ...
            CollectedData[@"lmsDBAUsers"] = result.ToString();
        }
        ...
        private static void SingleValueHandler
            (WindowsOracleInstanceLMSConcurrencyOptionsScript scriptInstance, String attributeName, String queryOutput)
        {
        ...
        /// <summary>
        /// Parse query result for Concurrency options.
        /// </summary>
        /// <param name="scriptInstance">script reference</param>
        /// <param name="attributeNames">attribute</param>
        /// <param name="queryOutput">Output</param>
        private static void ConcurrencyOptionsValueHandler
            (WindowsOracleInstanceLMSConcurrencyOptionsScript scriptInstance, String attributeName, String queryOutput)
        {
        ...
        /// </summary>
        private delegate void QueryResultHandler(WindowsOracleInstanceLMSConcurrencyOptionsScript scriptInstance, string attributeName, string outputData);
        ...
        /// </summary>
        static WindowsOracleInstanceLMSConcurrencyOptionsScript()
        {
        ...
        private static readonly KeyValuePair<string, QueryTableEntry>[] s_queryTable = {
            new KeyValuePair<string, QueryTableEntry>(@"VLicense",
            ...
            ver89_pattern,
            new QueryResultHandler(ConcurrencyOptionsValueHandler))),
            new KeyValuePair<string, QueryTableEntry>(@"VSession",
            new QueryTableEntry(@"SELECT '<BDNA=>SADDR='||SADDR||'<BDNA,1>SID='||SID||'<BDNA,1>PADDR='||PADDR||'<BDNA,1>USR='||USER#" +
                      "||'<BDNA,1>USERNAME='||USERNAME||'<BDNA,1>COMMAND='||COMMAND||'<BDNA,1>STATUS='||STATUS||'<BDNA,1>SERVER='||SERVER" +
                      "||'<BDNA,1>SCHEMANAME='||SCHEMANAME||'<BDNA,1>OSUSER='||OSUSER||'<BDNA,1>PROCESS='||PROCESS" +                      
                      ...
                      "||'<BDNA,1>TIMESTAMP='||SYSDATE||'<=BDNA>' FROM V$SESSION;",                      
            all_ver_pattern,
            new QueryResultHandler(ConcurrencyOptionsValueHandler))),
            new KeyValuePair<string, QueryTableEntry>(@"DBAUsers",
            new QueryTableEntry(@"SELECT '<BDNA=>USERNAME='||USERNAME||'<BDNA,1>USER_ID='||USER_ID||'<BDNA,1>DEFAULT_TABLESPACE='||DEFAULT_TABLESPACE" +
                     "||'<BDNA,1>TEMPORARY_TABLESPACE='||TEMPORARY_TABLESPACE||'<BDNA,1>CREATED='||CREATED||'<BDNA,1>PROFILE='||PROFILE" +
                     "||'<BDNA,1>EXPIRY_DATE='||EXPIRY_DATE||'<BDNA,1>TIMESTAMP='||SYSDATE||'<=BDNA>' FROM DBA_USERS;",                      
            all_ver_pattern,
            new QueryResultHandler(ConcurrencyOptionsValueHandler))),
            new KeyValuePair<string, QueryTableEntry>(@"lmsUserCount",
            new QueryTableEntry(@"select '<BDNA>lmsUserCount<BDNA>'||COUNT(*)||'<BDNA>' FROM DBA_USERS;",
            all_ver_pattern,
