...
namespace bdna.Scripts {
    public class BlackBerryHandheldL3MSSQLDatabaseScript : ICollectionScriptRuntime {
        ...
        public CollectionScriptResults ExecuteTask
            ...
            try {
                ...
                Lib.Logger.TraceEvent(TraceEventType.Start,
                                      0,
                                      "Task Id {0}: Collection script BlackBerryHandheldL3MSSQLDatabaseScript.",
                                      m_taskId);
                ...
                if (connection == null) {
                    ...
                    if (connection.ContainsKey("dbConnection")) {
                        ...
                        Lib.Logger.TraceEvent(TraceEventType.Error,
                                              0,
                                              "Task Id {0}: Missing dbConnection connection parameter.",
                                              m_taskId);
                    }
                    ...
                    if (connection.ContainsKey(@"databaseName")) {
                        ...
                        Lib.Logger.TraceEvent(TraceEventType.Error,
                                              0,
                                              "Task Id {0}: Missing databaseName connection parameter.",
                                              m_taskId);
                    }
                }
                ...
                if (dbConnection == null) {
                    ...
                    using (dbConnection) {
                        ...
                        resultCode = GetBlackBerryHandheldInfo(dbConnection, strDBName);
            ...
            Lib.Logger.TraceEvent(TraceEventType.Stop,
                                  0,
                                  "Task Id {0}: Collection script BlackBerryHandheldL3MSSQLDatabaseScript.  Elapsed time {1}.  Result code {2}.",
                                  m_taskId,
        ...
        private void BuildDataRow() {
            if (!m_attributes.ContainsKey(s_attributeName)) {
                Lib.Logger.TraceEvent(TraceEventType.Error,
                                      ...
                                      m_taskId,
                                      s_attributeName);
            } else if (0 == m_bbHandheldProperties.Count) {
                ...
                foreach (KeyValuePair<string, IDictionary<string, string>> record in m_bbHandheldProperties) {
        ...
        /// <summary>
        /// Collect information about all the blackberry handheld devices.
        /// </summary>
        /// 
        /// <param name="dbConnection">Connection to database server.</param>
        /// <returns>Operation result code.</returns>
        private ResultCodes GetBlackBerryHandheldInfo(DbConnection dbConnection, string strDBName) {
            ...
            try {
                Lib.Logger.TraceEvent(TraceEventType.Information,
                                      0,
                                      "Task Id {0}: Executing select sql against dbo.vHandheldSummaryInfo {1} on {2}.",
                                      m_taskId,
                                      dbConnection.Database,
                                      dbConnection.DataSource);
                sw.Start();
                DbCommand cmd = dbConnection.CreateCommand();
                cmd.CommandText = @"select * from "+ strDBName + ".dbo.vHandheldSummaryInfo";
                ...
                using (reader) {
                    for (recordCount = 0; reader.Read(); recordCount++) {
                        ...
                        if (!String.IsNullOrEmpty(userName)) {
                            ...
                            this.addBlackBerryHandheldProperties(userName, record);
                        }
                    }
                }
        ...
        private void addBlackBerryHandheldProperties(string databaseName, IDictionary<string, string> details) {
            m_bbHandheldProperties[databaseName] = details;
        }
        private IDictionary<string, IDictionary<string, string>> m_bbHandheldProperties
            = new Dictionary<string, IDictionary<string, string>>();

        /// <summary>Database server name.</summary>
        private string m_dbServerName;

        static string s_attributeName = @"BlackBerryHandheldL3Details";
    }
}
