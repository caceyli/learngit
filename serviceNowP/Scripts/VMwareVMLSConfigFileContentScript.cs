...
/******************************************************************
*
*          Module: VMware VMLS Config File Dynamic Collection Scripts
...
namespace bdna.Scripts {
    public class VMwareVMLSConfigFileContentScript : ICollectionScriptRuntime {
        /// <summary>
        ...
        /// <summary>
        /// Format VMLS file content to name=value pair format for easy rule processing.
        /// </summary>
        /// 
        ...
        private string formatFileContent(string strOrigFileContent) {
            ...
            if (!String.IsNullOrEmpty(strOrigFileContent)) {
                ...
                foreach (string strOneLine in strLines) {
                    ...
                    if (s_pattern.IsMatch(strLine)) {
                        ...
                        if (!string.IsNullOrEmpty(strName)) {
                            if (strFormattedFileContent.Length > 0) {
                                if (strName.Equals(@"config", StringComparison.InvariantCultureIgnoreCase)) {
                                    strFormattedFileContent.Append(BdnaDelimiters.DELIMITER_TAG);
                                }
                                else {
                                    strFormattedFileContent.Append(BdnaDelimiters.DELIMITER1_TAG);
                                }
                            }
                    ...
                    if (s_pattern2.IsMatch(strLine)) {
                        Match match = s_pattern2.Match(strLine);
                        string strVmxFilePath = match.Groups[@"FileName"].ToString();

                        if (!string.IsNullOrEmpty(strVmxFilePath)) {
                            if (strFormattedFileContent.Length > 0) {
                                strFormattedFileContent.Append(BdnaDelimiters.DELIMITER_TAG);
                            }
                            strFormattedFileContent.Append(@"FilePath=""").Append(strVmxFilePath).Append(@"""");
                        }
                    }
                }
            }
        ...
        /// <summary>VMware VMLS File path. </summary>
        private string m_vmlsFilePath;
        ...
        private static Regex s_pattern = new Regex(@"vmlist\d+\.(?<name>.+?)\s*=\s*""(?<value>.*)\""",
                                                   RegexOptions.Compiled | RegexOptions.IgnoreCase);
        private static Regex s_pattern2 = new Regex(@"config\s+""(?<FileName>.+)""",
                                                    RegexOptions.Compiled | RegexOptions.IgnoreCase);
