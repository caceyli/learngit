...
namespace bdna.Scripts {
    public class Extend_IBM_WindowsWebSphereMQ_Server_QueueManagerStatic_script : ICollectionScriptRuntime {
        /// <summary>
        ...
        private string formatCommandResult (string strOrigCommandResult) {
            ...
            if (!String.IsNullOrEmpty(strOrigCommandResult)) {
                ...
                foreach (string strOneLine in strLines) {
                    ...
                    if (strOneLine.Contains(@"runmqsc") || strOneLine.Contains(@"Copyright IBM Corp") || strOneLine.Contains(@"Starting MQSC")) continue;
                    string s = Regex.Replace(strOneLine, @"^\s*(.*?)\s*$", "$1");

                    if (strFormattedCommandResult.Length > 0) {
                        strFormattedCommandResult.Append(BdnaDelimiters.DELIMITER_TAG);
                    }
                    //strFormattedCommandResult.Append(strOneLine);
