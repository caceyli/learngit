...
namespace bdna.Scripts {
    public class WindowsOracleInstanceDynamicScript : ICollectionScriptRuntime {
        ...
        public CollectionScriptResults ExecuteTask(
            ...
            Lib.Logger.TraceEvent(TraceEventType.Start,
                                  0,
                                  "Task Id {0}: Collection script WindowsOracleInstanceDynamicScript.",
                                  m_taskId);
            ...
            try {
                ...
                if (m_connection == null) {
                    ...
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Connection object passed to WindowsOracleInstanceDynamicScript is null.",
                                          m_taskId);
                } 
                    ...
                    using (IRemoteProcess rp = RemoteProcess.ExecuteBatchFile
                        ...
                        if (resultCode == ResultCodes.RC_SUCCESS) {
                            ...
                            if (rp.Stdout != null && rp.Stdout.Length > 0) {
                                ...
                                else if (!rp.Stdout.ToString().Contains(@"-------")) {
                                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                                          0,
            ...
            catch (Exception ex)
            {
                if (ResultCodes.RC_SUCCESS == resultCode) {
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Unhandled exception in WindowsOracleInstanceDynamicScript.  Elapsed time {1}.\n{2}\nResult code changed to RC_PROCESSING_EXCEPTION.",
                                          m_taskId,
            ...
            Lib.Logger.TraceEvent(TraceEventType.Stop,
                                  0,
                                  "Task Id {0}: Collection script WindowsOracleInstanceDynamicScript.  Elapsed time {1}.  Result code {2}.",
                                  m_taskId,
                                  m_executionTimer.Elapsed.ToString(),
                                  resultCode.ToString());
        ...
        private static void SingleValueAccumulationHandler
            (WindowsOracleInstanceDynamicScript scriptInstance, String attributeName, String queryOutput) {
        ...
        /// </summary>
        private delegate void QueryResultHandler(WindowsOracleInstanceDynamicScript scriptInstance, string attributeName, string outputData);
        ...
        /// </summary>
        static WindowsOracleInstanceDynamicScript() {
        ...
        private static readonly KeyValuePair<string, QueryTableEntry>[] s_queryTable = {
            new KeyValuePair<string, QueryTableEntry>(@"schemas",
            new QueryTableEntry(@"select '<BDNA>schemas<BDNA>' ||username||'<BDNA>' from dba_users;",
                                new QueryResultHandler(SingleValueAccumulationHandler))),
            new KeyValuePair<string, QueryTableEntry>(@"tablespaces",
            new QueryTableEntry(@"select '<BDNA>tablespaces<BDNA>' ||tablespace_name||'<BDNA>' from dba_tablespaces;",
                                new QueryResultHandler(SingleValueAccumulationHandler)))
        };
