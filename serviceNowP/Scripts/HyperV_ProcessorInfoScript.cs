...
{
    public class HyperV_ProcessorInfoScript : ICollectionScriptRuntime
    {
        /// <summary>
                ...
                ITftpDispatcher tftpDispatcher) 
        {
            ...
            Lib.Logger.TraceEvent(TraceEventType.Start,
                                  0,
                                  "Task Id {0}: Collection script HyperV_ProcessorInfoScript.",
                                  m_taskId);
            try
            {
                ...
                if (connection == null)
                {
                    ...
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Connection object passed to HyperV_ProcessorInfoScript is null.",
                                          m_taskId);
                }
     ...
     /*                         ConnectionOptions connectionOptions = new ConnectionOptions();
                    ...
                    String queryString = @"SELECT * FROM Msvm_ProcessorSettingData WHERE InstanceID like '%" + m_VM_Guid + @"%'";
                    ...
                    if (ResultCodes.RC_SUCCESS == resultCode && null != moc)
                      {
                          using (moc)
                          {
                              ...
                              {
                                  procWeight = Convert.ToString(mo["Weight"]);
                                  procReservation = Convert.ToString(mo["Reservation"]);
                                  procCount = Convert.ToString(mo["VirtualQuantity"]);
                                  guid = (String)mo["InstanceID"];
                                  guid = guid.Replace("Microsoft:", "");
                                  guid = guid.Substring(0, 36);
                                  sbVmInfo.Append(@"<BDNA,>Guid=""" + guid + @"""<BDNA,1>procWeight=""" + procWeight + @"""<BDNA,1>procReservation=""" + procReservation + @"""<BDNA,1>procCount=""" + procCount + @"""");
                              }
            ...
            catch (ManagementException mex)
            {
                if (resultCode == ResultCodes.RC_SUCCESS)
                {
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: ManagementException in HyperV_ProcessorInfoScript.  Elapsed time {1}.\n{2}\nResult code changed to RC_PROCESSING_EXCEPTION.",
                                          m_taskId,
            ...
            catch (Exception ex)
            {
                if (resultCode == ResultCodes.RC_SUCCESS)
                {
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Unhandled exception in HyperV_ProcessorInfoScript.  Elapsed time {1}.\n{2}\nResult code changed to RC_PROCESSING_EXCEPTION.",
                                          m_taskId,
                                          m_executionTimer.Elapsed.ToString(),
                                          ex.ToString());
                    resultCode = ResultCodes.RC_PROCESSING_EXCEPTION;
            ...
            Lib.Logger.TraceEvent(TraceEventType.Stop,
                                  0,
                                  "Task Id {0}: Collection script HyperV_ProcessorInfoScript.  Elapsed time {1}.  Result code {2}.",
                                  m_taskId,
                                  m_executionTimer.Elapsed.ToString(),
                                  resultCode.ToString());
        ...
        public static string s_attributeName = @"processInfo";
