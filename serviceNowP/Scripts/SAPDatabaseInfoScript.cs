...
/******************************************************************
*
*          Module: SAP Client Info Scripts
...
namespace bdna.Scripts {
    public class SAPDatabaseInfoScript : ICollectionScriptRuntime {
        ...
        public CollectionScriptResults ExecuteTask
            ...
            try {
                Lib.Logger.TraceEvent(TraceEventType.Start,
                                      0,
                                      "Task Id {0}: Collection script SAPDatabaseInfoScript.",
                                      m_taskId);
                ...
                if (connection == null) {
                    ...
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Connection object passed to SAPDatabaseInfoScript is null.",
                                          m_taskId);
                }
                else {
                ...
                if (resultCode == ResultCodes.RC_SUCCESS) {
                    using (SAPGuiConnection oConn = new SAPGuiConnection()) {
                        if (!String.IsNullOrEmpty(SAPConnectionString)) {
                            ...
                            Lib.Logger.TraceEvent(TraceEventType.Verbose,
                                                  0,
                                                  "Task Id {0}: Collecting SAP database information from remote SAP server",
                                                  m_taskId);
                            sw.Reset();
                            sw.Start();
                            SAPDatabaseInfoCollector oDatabaseInfoCollector = new SAPDatabaseInfoCollector(ref oConn.Session);
                            oDatabaseInfoCollector.Collect();
                            m_sapDatabaseInfoProperties = oDatabaseInfoCollector.ResultString;
                            Lib.Logger.TraceEvent(TraceEventType.Verbose,
                                                  0,
                                                  "Task Id {0}: Collection of database information complete.  Elapsed time {1}.\n{2}",
                                                  m_taskId,
                                                  sw.Elapsed.ToString(),
                                                  oDatabaseInfoCollector.logData.ToString());
                        }                        
                    }
                    BuildDataRow();
                }
            ...
            catch (Exception ex)
            {
                if (resultCode == ResultCodes.RC_SUCCESS) {
                    Lib.Logger.TraceEvent(TraceEventType.Error,
                                          0,
                                          "Task Id {0}: Unhandled exception in SAPDatabaseInfoScript.  Elapsed time {1}.\n{2}\nResult code changed to RC_PROCESSING_EXCEPTION.",
                                          m_taskId,
            ...
            Lib.Logger.TraceEvent(TraceEventType.Stop,
                                  0,
                                  "Task Id {0}: Collection script SAPDatabaseInfoScript.  Elapsed time {1}.  Result code {2}.",
                                  m_taskId,
                                  m_executionTimer.Elapsed.ToString(),
                                  resultCode.ToString());
        ...
        private void BuildDataRow() {
            if (!m_attributes.ContainsKey(s_attributeName)) {
                Lib.Logger.TraceEvent(TraceEventType.Error,
                                      ...
                                      m_taskId,
                                      s_attributeName);
            } else if (String.IsNullOrEmpty(m_sapDatabaseInfoProperties)) {
                Lib.Logger.TraceEvent(TraceEventType.Verbose,
                                      0,
                ...
                m_dataRow.Append(m_elementId).Append(',')
                         ...
                         .Append(m_sapDatabaseInfoProperties)
        ...
        /// Collected Data
        /// </summary>
        private string m_sapDatabaseInfoProperties = String.Empty;

        static string s_attributeName = @"__SAPDatabaseDetails";
    }
}
